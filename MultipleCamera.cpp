...
// image_transport::Publisher pubImages[MAX_DEVICE_NUM];
ros::Publisher pubImages[MAX_DEVICE_NUM];
static io_context ioContext_;
...
class TIMESYNC_TCPClient { // 用于处理时间同步和IMU数据的TCP客户端
  ...
  static constexpr int msg_len_ = 512;

public:
  TIMESYNC_TCPClient(const std::string &ip, int port)
      : ip_address_(ip), port_(port), tcp_stream_() {
    connect();
  }
  ...
  void timestamp_proc() { // 处理时间戳数据timestamp_proc
    ...
    while (!exit_flag && tcp_stream_.getline(buf_, msg_len_)) {
...
// 图片转化
// Image convert
static void imageConvert(CameraInfo *pCameraInfo, IMV_Frame frame,
                         IMV_EPixelType convertFormat) {
  ...
  const char *cvPixelFormatStr = NULL;

  switch (convertFormat) {
  case gvspPixelRGB8:
    nDstBufSize = sizeof(unsigned char) * frame.frameInfo.width *
  ...
  default:
    nDstBufSize =
        sizeof(unsigned char) * frame.frameInfo.width * frame.frameInfo.height;
  ...
  if (IMV_OK == ret) {
    cv::Mat srcImage(stPixelConvertParam.nHeight, stPixelConvertParam.nWidth,
                     CV_8UC1, stPixelConvertParam.pDstBuf);
    /// 灰度：mono8
    /// 彩色：bgr8
    sensor_msgs::ImagePtr msg =
        cv_bridge::CvImage(std_msgs::Header(), cvPixelFormatStr, srcImage)
            .toImageMsg();
    ...
    std::cout << pubImages[pCameraInfo->carmeraId].getTopic()
              << " published: " << msg->header.stamp.toSec() << std::endl;
  } else {
...
static void onGetFrame(IMV_Frame *pFrame, void *pUser) {
  ...
  // imageConvert(pCameraInfo, *pFrame, convertFormat);

  cv::Mat srcImage = cv::Mat(pFrame->frameInfo.height, pFrame->frameInfo.width,
                             CV_8UC1, pFrame->pData);
  ...
  /// 彩色：bgr8
  sensor_msgs::ImagePtr msg =
      cv_bridge::CvImage(std_msgs::Header(), "mono8", srcImage).toImageMsg();
...
int openCamera(MultipleCamera *pMultipleCameraInfo, ros::NodeHandle &nh,
  ...
  for (index = 0; index < pMultipleCameraInfo->nOpenCameraNum; index++) {
    ...
    ret = IMV_GetDeviceInfo(
        devHandle, &pMultipleCameraInfo->cameraInfo[openCameraNum].info);
    if (IMV_OK != ret) {
    ...
    if (IMV_OK != ret) {
      printf("Open camera1 failed! cameraKey[%s], ErrorCode[%d]\n",
             pMultipleCameraInfo->cameraInfo[openCameraNum].info.cameraKey,
             ret);
    ...
    pubImages[openCameraNum] = nh.advertise<sensor_msgs::Image>(
        "cam" + std::to_string(openCameraNum), 1);
...
int startGrabbing(MultipleCamera *pMultipleCameraInfo) {
  ...
  for (index = 0; index < pMultipleCameraInfo->nOpenCameraNum; index++) {
    ...
    ret = IMV_AttachGrabbing(pMultipleCameraInfo->cameraInfo[index].handle,
                             onGetFrame,
                             (void *)&pMultipleCameraInfo->cameraInfo[index]);
    if (IMV_OK != ret) {
...
int main(int argc, char **argv) {
  ...
  if (!cam_node.tcp_stream_ || !imu_node.tcp_stream_) {
    std::cerr << "Failed to connect to the time sync or IMU server."
              << std::endl;
    return -1;
  }
